...
// compile by
// g++ Demo_GA3Ezx.cp -l ginac -l cln
...
void PrintSimpleBlade(GA3Ezx a) 
{
	int Count = 0;
...
int	GetWeight(GA3Ezx r) // assume only one non-zero component
{
...
GA3Ezx SymmetricViaFormula(GA3Ezx a, GA3Ezx b)
{
	GA3Ezx  c,d,e,f;
...
GA3Ezx AntiSymmetricViaFormula(GA3Ezx a, GA3Ezx b)
{
	GA3Ezx  c,d,e,f;
...
int main(void)
{
	...
	symbol c_xy("c.e"),  c_zx("c.f"),  c_yz("c.g");
	symbol c_xyz("c.h");

	symbol d_q("d.a");
	symbol d_x("d.b") ,  d_y("d.c") ,  d_z("d.d");
	symbol d_xy("d.e"),  d_zx("d.f"),  d_yz("d.g");
	symbol d_xyz("d.h");
...
//	ex q,  x,y,z, xy,zx,yz,  xyz;
	...
	q    = GA3Ezx(1, 0,0,0, 0,0,0, 0);

	x    = GA3Ezx(0, 1,0,0, 0,0,0, 0);
	y    = GA3Ezx(0, 0,1,0, 0,0,0, 0);
	z    = GA3Ezx(0, 0,0,1, 0,0,0, 0);

	xy   = GA3Ezx(0, 0,0,0, 1,0,0, 0);
	zx   = GA3Ezx(0, 0,0,0, 0,1,0, 0);
	yz   = GA3Ezx(0, 0,0,0, 0,0,1, 0);

	xyz  = GA3Ezx(0, 0,0,0, 0,0,0, 1);
...
// we now print out the constituent equations for C = A B using s x y z xy zx yz xyz format
...
// test Dual
	...
	s = GA3Ezx(0,  0,0,0,  0,0,0, -1);   // s = I_inv
	u = r*s;
	t = Dual(r);
